...
def retrieve_and_open_csv_files(folder_path, keyword=None):
    ...
    if len(csv_files) != 1:
        ...
        dataframes = pd.read_csv(csv_files[0], encoding = '1254')
        if 'Timestamp' in list(dataframes.columns):
            dataframes = pd.read_csv(csv_files[0], encoding = '1254', parse_dates = ['Timestamp'])
        elif 'Start time' in list(dataframes.columns):
            dataframes = pd.read_csv(csv_files[0], encoding = '1254', parse_dates = ['Start time'])
